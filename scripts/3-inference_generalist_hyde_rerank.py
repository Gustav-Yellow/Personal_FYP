...
def retrieve_rag_content_hybrid(embed_query, rerank_query, task_type):
    ...
    # ================= 1. 常识库混合检索 =================
    D, I = cs_index.search(query_vec, k=10) # 粗排捞 10 条
    candidates = []
...
def process_dataset(task_type, input_filename, output_filename, chat_model, instruction):
    ...
    with open(input_path, 'r', encoding='utf-8') as f:
        ...
        random.seed(42) # 固定随机种子，保证每次测试的 50 题一样
...
"""
    ...
    print(f"✅ 混合模式(HyDE+Rerank)结果已保存至: {output_path}")
...
if __name__ == "__main__":
    ...
    print("\n=== 开始推理 (Generalist HyDE + Rerank 终极模式) ===")
    process_dataset("CR", "test_CRMUS_CR_manual.json", "test_CRMUS_CR_pred_generalist_hyde_rerank.json", chat_model, CR_INSTRUCTION)
    process_dataset("MU", "test_CRMUS_MU_manual.json", "test_CRMUS_MU_pred_generalist_hyde_rerank.json", chat_model, MU_INSTRUCTION)